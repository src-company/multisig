...
7. **Coordination database** — Not trusted for anything the chain settles, and not an authority on any of it. The vault decides ownership, thresholds, nonces and what has executed; the client re-derives every EIP-712 digest from a row's own fields and recovers every signature against the chain's owner set, so a forged or altered row cannot be signed into existence or displayed as signed. What the database is trusted for is availability and integrity of *coordination* — proposals reaching co-signers, and the two display fields (a vault name, an owner label) that exist nowhere else. Proposal admission and those two fields require a verified owner signature; the remaining write RPCs still accept a claimed caller address, and are bounded rather than authenticated: every column they touch is re-derived from chain on the next load, so the exposure is noise and delay, never theft. An operator running this stack should assume anyone can read it — vault names, owner labels, proposal descriptions and collected signatures are world-readable — and should apply `db/roles.sql` alongside `db/schema.sql`, since it is the file that keeps the non-re-derivable writes off the anonymous role.
