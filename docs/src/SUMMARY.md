# Summary

[Multisig](README.md)

# Protocol

- [Overview](protocol/overview.md)
- [Deployment](protocol/deployment.md)
- [Signatures](protocol/signatures.md)
- [Timelock](protocol/timelock.md)
- [The Executor Role](protocol/executor.md)
- [Modules](protocol/modules.md)
- [Comparison with Safe](protocol/comparison.md)
- [Security](protocol/security.md)

# Interface

- [The Interface](dapp/overview.md)
- [Using the Interface](dapp/interface.md)
- [Guardrails](dapp/guardrails.md)
- [Architecture](dapp/architecture.md)

# Reference

- [Contract Reference](reference.md)
  - [Multisig](src/Multisig.sol/contract.Multisig.md)
  - [MultisigFactory](src/Multisig.sol/contract.MultisigFactory.md)
  - [TimelockExecutor](src/mods/TimelockExecutor.sol/contract.TimelockExecutor.md)
  - [AllowlistGuard](src/mods/AllowlistGuard.sol/contract.AllowlistGuard.md)
  - [SpendingAllowance](src/mods/SpendingAllowance.sol/contract.SpendingAllowance.md)
  - [SocialRecovery](src/mods/SocialRecovery.sol/contract.SocialRecovery.md)
  - [DeadmanSwitch](src/mods/DeadmanSwitch.sol/contract.DeadmanSwitch.md)
  - [IMultisig](src/mods/interfaces/IMultisig.sol/interface.IMultisig.md)
